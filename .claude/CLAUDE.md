...
-   **Never fall back to `any` when stuck.** If you cannot confidently derive the right type (unclear library types, complex generics, external data shape), stop and ask the user rather than reaching for `any` or an unchecked `as` cast. `unknown` + a narrowing guard is acceptable at true boundaries; silent `any` is not.

-   **Never use `!` (non-null assertion).** Use `ensure(value, "message")` from `src/utils/assert.ts` instead — it narrows the type at runtime with a real check, so a bug surfaces as a clear error instead of a silent `undefined` downstream. `!` is banned even when you're "sure" the value is present; if it's truly guaranteed by construction, restructure so the type reflects that.
